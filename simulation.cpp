...
void simulate(char* modelF, char* resultsF, bool deterministic = false) {
  ...
  std::vector<int> sim_counts;
    ...
    for (int i = 1; i <= M; i++) theta[i] = theta[i] / denom * (1.0 - theta[0]);
	  ...
	  // the "deterministic" case where we simply want to simulate reads

    if (alleleS) {
      std::cerr << "ERROR: Allele specific expression for deterministic " <<
        "simulations has not been implemented yet." << std::endl;
      exit(1);
    }
    ...
    const size_t N_DISCARD = 4;
      ...
      // skip the first few cols
      for (size_t d = 0; d < N_DISCARD; ++d) {
        ss >> discard;
      }

      ss >> cur_count;
      // cout << "the current count " << cur_count << endl;
      sim_counts.push_back( cur_count );

      // ignore the rest of the columns
	  }

    // just give a dummy value to theta so it doesn't complain
	  double unif = 1 / (M + 1);
	  for (int i = 0; i <= M; ++i) {
	    theta[i] = unif;
	  }
...
int main(int argc, char* argv[]) {
	...
	switch(model_type) {
	case 0: simulate<SingleRead, SingleModel>(argv[2], argv[3], deterministic); break;
	case 1: simulate<SingleReadQ, SingleQModel>(argv[2], argv[3], deterministic); break;
	case 2: simulate<PairedEndRead, PairedEndModel>(argv[2], argv[3], deterministic); break;
	case 3: simulate<PairedEndReadQ, PairedEndQModel>(argv[2], argv[3], deterministic); break;
	}

  // TODO: convert counts to an integral type
